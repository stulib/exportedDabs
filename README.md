...
- `${var.workspace_host}` for each target
- `${var.catalog}` — the default Unity Catalog catalog (defaults to `classic_stable_aiy0te`)
- `${var.schema}` — the default Unity Catalog schema (defaults to `dabs_test`)
- `${var.warehouse_id}` — the SQL warehouse ID
...
- `catalog` — Unity Catalog catalog name (mapped to `BUNDLE_VAR_catalog`)
- `schema` — Unity Catalog schema name (mapped to `BUNDLE_VAR_schema`)
